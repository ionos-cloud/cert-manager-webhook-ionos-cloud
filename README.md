...
The Helm chart defaults to the version defined in chart `appVersion`. You can still force a specific image tag using `--set image.tag=$VERSION --set image.override=true`.
